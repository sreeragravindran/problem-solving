package coding.threading.uberRideProblem;
...
class UberSeatingProblem {
    ...
    void seatDemocrat() throws InterruptedException, BrokenBarrierException {
        ...
        if (democrats == 4) {
            ...
            lock.unlock();
            // wait until notified to proceed for seating
            demsWaiting.acquire();
        }

        // when the waiting threads at ( line #48 ) are woken up ( by the rideLeader holding the lock) ,
        // they do not need lock to execute the below block
        // hence, along with the rideLeader thread, these are also run in parallel and go wait at the barrier( analogous to getting seated )
        seated();
        barrier.await();
